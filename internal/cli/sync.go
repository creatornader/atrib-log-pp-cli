...
import (
	"atrib-log-pp-cli/internal/cliutil"
...
Resource scoping:
  ...
  atrib-log-pp-cli sync

  # Sync specific resources only
  atrib-log-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  atrib-log-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  atrib-log-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  atrib-log-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  atrib-log-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/atrib-log-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "by-context":
		return db.UpsertByContext(data)
	case "lookup":
		return db.UpsertLookup(data)
	case "stats":
		return db.UpsertStats(data)
	case "tile":
		return db.UpsertTile(data)
	default:
