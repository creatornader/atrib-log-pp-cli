...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".atrib-log-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ATRIB_LOG_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ATRIB_LOG_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "atrib-log-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.atrib-log-pp-cli/feedback.jsonl.
When ` + "`ATRIB_LOG_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ATRIB_LOG_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  atrib-log-pp-cli feedback list
  atrib-log-pp-cli feedback list --limit 5
  atrib-log-pp-cli feedback list --json`,
